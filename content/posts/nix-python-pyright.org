...
So, a subpar approach would be to hardcode that to the location in the =/nix/store= that
...
So everytime you open your project, direnv sees =use_nix= in =.envrc=, resolves your =shell.nix= file, and injects any environment variables (=PYTHONPATH=) into your editor, using your direnv editor plugin.
